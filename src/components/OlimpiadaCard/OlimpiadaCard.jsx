...
export function OlimpiadaCard(props) {
  ...
  const btnClick = () => navigate(`/aluno/olimpiada/${props.id}`)
  ...
  return (
    <div className={"olimpiada " + props.classe}>
      ...
      { props.isAluno && <BotaoPrincipal classe={(allowAccess ? '' : 'disabled') + ' btn-md-olimpiada'} btnClick={allowAccess ? btnClick : null} disabled={false}>{ isLoading ? <div className="spinner"></div> : 'Acessar' }</BotaoPrincipal> }
    </div>
  )
}
